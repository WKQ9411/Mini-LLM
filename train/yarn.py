...
def parse_args():
    parser = argparse.ArgumentParser(description="YaRN Mini-LLM")
    ...
    # YaRN 配置
    parser.add_argument("--max_seq_len", type=int, default=2048, help="Extended maximum pretraining sequence length")
    parser.add_argument("--yarn_alpha", type=float, default=1, help="YaRN alpha parameter")
    parser.add_argument("--yarn_beta", type=float, default=32, help="YaRN beta parameter")
    
    # 训练参数设置(默认配置，mini_llama3 单卡大约占用将近50GB显存)
    parser.add_argument("--max_batch_size", type=int, default=8, help="Training batch size per GPU")
    ...
    # 路径和日志设置
    parser.add_argument("--yarn_data_path", type=str, default=f"{root_path}/data/pretrain_data/bin/fineweb_edu_sampled_0.1_percent.bin", help="Path to YaRN dataset")
...
def train_process(local_rank, rank, world_size, args):
    ...
    # ------------------ 2. 数据准备 ------------------
    dataset = PreTrainDataset(file_path=args.yarn_data_path, max_seq_len=args.max_seq_len)
    ...
    if args.resume_from_checkpoint and os.path.exists(args.resume_from_checkpoint):
        ...
        if not os.path.exists(config_path):
            raise FileNotFoundError(f"config.json not found in checkpoint directory: {args.resume_from_checkpoint}")
        ...
        # 如果不是从 checkpoint 恢复，先基于预训练配置注入 YaRN 参数，再加载权重
    ...
    if is_main_process:
        ...
        model_name = f"yarn_{args.model_name}"
        
        # 如果从 checkpoint 恢复，使用 checkpoint 的父目录作为输出路径
        if args.resume_from_checkpoint and os.path.exists(args.resume_from_checkpoint):
            # 从 checkpoint 路径推断输出路径
            # checkpoint 路径格式: output/yarn_model_name/checkpoints/checkpoint-XXXXX
            checkpoint_parent = os.path.dirname(args.resume_from_checkpoint)  # checkpoints 目录
            current_train_path = os.path.dirname(checkpoint_parent)  # yarn_model_name 目录
            print(f"Resuming training, using output path: {current_train_path}")
        else:
            current_train_path = os.path.join(args.output_path, model_name)  # ./output/yarn_model_name
        ...
        if not os.path.exists(log_dir):
            os.makedirs(log_dir)
        writer = SummaryWriter(log_dir=log_dir)  # ./output/yarn_model_name/logs
